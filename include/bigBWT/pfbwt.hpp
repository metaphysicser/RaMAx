...
namespace PfBWT {
     ...
     * ******************************************************************* */
    static void bwt(Args& arg, uint8_t* d, long dsize, // dictionary and its size  
        ...
        uint32_t* istart, long dwords) // starting point in ilist for each word and # words
    {
        ...
        if (arg.SA) safile = open_aux_file(arg.filename, EXTSA, "wb");
        if (arg.sampledSA & START_RUN) ssafile = open_aux_file(arg.filename, EXTSSA, "wb");
        if (arg.sampledSA & END_RUN) esafile = open_aux_file(arg.filename, EXTESA, "wb");
        ...
        // open output file
        FILE* fbwt = open_aux_file(arg.filename, "bwt", "wb");
    ...
    static uint8_t* load_bwsa_info(Args& arg, long n)
    {
        ...
        FILE* fin = open_aux_file(arg.filename, EXTBWSAI, "rb");
