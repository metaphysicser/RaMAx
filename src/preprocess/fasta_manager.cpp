...
// FastaManager::FastaManager(const FilePath& fasta_path, const FilePath&
// fai_path)
...
void FastaManager::loadFaiRecords(const FilePath &fai_path) {
  ...
  while (std::getline(in, line)) {
    ...
    FaiRecord rec;
    iss >> rec.seq_name >> rec.global_start_pos >> rec.length >> rec.offset >>
        rec.line_bases >> rec.line_bytes;

    if (iss.fail()) {
      spdlog::warn("Skipping malformed line in {}: {}", fai_path.string(),
                   line);
      continue;
    }
...
FilePath FastaProcessor::writeCleanedFasta(const FilePath &output_file,
                                           uint64_t line_width) {
...
std::string FastaManager::getSubConcatSequence(size_t start, size_t length) {
  ...
  for (const auto &rec : fai_records) {
    // 每条序列在全局坐标中的范围是 [current_offset, current_offset + rec.length
    // - 1]
    ...
    if (start <= seq_end_global && (start + remain - 1) >= seq_start_global) {
      ...
      // 例如，如果 start=1500 而这条序列 global 区间 [1000..2999]，则
      // local_start=1500-1000=500
      size_t local_start =
          (start > seq_start_global) ? (start - seq_start_global) : 0;
      ...
      // 我们实际要从本序列读出的长度
      size_t to_read =
          (remain < can_read_in_this_seq) ? remain : can_read_in_this_seq;
  ...
  if (remain > 0) {
    throw std::runtime_error(
        "Requested range exceeds total length of all sequences.");
  }
...
std::string FastaManager::getSubSequence(const std::string &seq_name,
  ...
  if (!find) {
    throw std::runtime_error("Cannot find sequence " + seq_name +
                             " in FAI records.");
  }
  ...
  if (fd == -1) {
    throw std::runtime_error("Failed to open " + fasta_path_.string() +
                             " for reading sub-sequence.");
  }
  ...
  if (file_size == -1) {
    close(fd);
    throw std::runtime_error("Failed to determine the size of " +
                             fasta_path_.string());
  }
  ...
  if (to_read > 0) {
    throw std::runtime_error(
        "Reached EOF unexpectedly while reading sub-sequence for " + seq_name);
  }
...
RegionVec FastaManager::preAllocateChunks(uint_t chunk_size,
                                          uint_t overlap_size) {
  RegionVec chunks;
  ...
  for (const auto &rec : fai_records) {
    ...
    while (start < chr_len) {
      ...
      size_t this_len = std::min<size_t>(chunk_size, chr_len - start);
...
// intervals_by_seq: 按序列名称组织的区间映射，格式为 {seq_name: [(start1,
// end1), (start2, end2), ...]} output_fasta_path:
// 输出的隐藏区间后的FASTA文件路径 output_fai_path: 输出的FAI索引文件路径
...
bool FastaManager::hideIntervalsAndGenerateFai(
    const std::map<std::string, std::vector<std::pair<size_t, size_t>>>
        &intervals_by_seq,
    const FilePath &output_fasta_path, const FilePath &output_fai_path,
    size_t line_width) {

  try {
    // 1. 复制区间数据并进行处理
    std::map<std::string, std::vector<std::pair<size_t, size_t>>>
        processed_intervals = intervals_by_seq;
    ...
    if (!out_fasta.is_open()) {
      spdlog::error("Failed to create output FASTA file: {}",
                    output_fasta_path.string());
      return false;
    }

    std::ofstream out_fai(output_fai_path);
    if (!out_fai.is_open()) {
      spdlog::error("Failed to create output FAI file: {}",
                    output_fai_path.string());
      return false;
    }
    ...
    while (nextRecord(header, sequence)) {
      ...
      if (hide_intervals.empty()) {
        ...
        for (const auto &interval : hide_intervals) {
          ...
          if (current_pos < hide_start) {
            ...
            if (segment_start < sequence.length() &&
                segment_end < sequence.length()) {
              std::string segment = sequence.substr(
                  segment_start, segment_end - segment_start + 1);
      ...
      for (size_t i = 0; i < segments.size(); ++i) {
        ...
        // 写入FAI记录
        out_fai << segment_name << "\t" << global_start_pos << "\t"
                << segment.length() << "\t" << seq_offset << "\t" << line_bases
                << "\t" << line_bytes << "\n";
    ...
    spdlog::info("Successfully generated hidden intervals FASTA: {}",
                 output_fasta_path.string());
    spdlog::info("Successfully generated corresponding FAI index: {}",
                 output_fai_path.string());
...
bool FastaManager::hideIntervalsFromFileAndGenerateFai(
    const FilePath &interval_file_path, const FilePath &output_fasta_path,
    const FilePath &output_fai_path, size_t line_width) {
  ...
  try {
    ...
    if (!interval_stream.is_open()) {
      spdlog::error("Failed to open interval file: {}",
    ...
    while (std::getline(interval_stream, line)) {
      ...
      if (line[0] == '>') {
        ...
        // 清理序列名称中的空白字符
        current_seq_name.erase(
            0, current_seq_name.find_first_not_of(" \t\n\r\f\v"));
        current_seq_name.erase(
            current_seq_name.find_last_not_of(" \t\n\r\f\v") + 1);
        ...
        try {
          ...
          spdlog::warn("Skipping invalid interval line: {} - {}", line,
                       e.what());
        }
      }
    }
    ...
    // 2. 调用通用的隐藏区间函数
    return hideIntervalsAndGenerateFai(intervals_by_seq, output_fasta_path,
                                       output_fai_path, line_width);
