...
bool FM_Index::newScan(const FilePath &fasta_path, const FilePath &output_path,
                       uint_t thread) {
    ...
    if (std::filesystem::exists(occ_path) && std::filesystem::exists(dict_path) &&
        std::filesystem::exists(parse_path)) {
        return true;
    }
...
bool FM_Index::bwtParse(const FilePath &fasta_path, const FilePath &output_path,
                        uint_t thread) {
    ...
    if (std::filesystem::exists(last_path) || std::filesystem::exists(sai_path) ||
        std::filesystem::exists(ilist_path)) {
        return true;
    }
    ...
    char *filename_char =
            new char[filename_str.length() + 1]; // Create a modifiable char array
    std::strcpy(filename_char,
                filename_str.c_str()); // Copy the string into the char array
    ...
    BWTsa[0] = Text[n - 1];
    if (fputc(last[n - 2], lastout) == EOF)
        die("bwlast output 1");
    if (args.SAinfo)
        get_and_write_myint(sa_info, n, n - 1, sa_out);

    for (long i = 1; i <= n; i++) {
        if (SA[i] == 0) {
            assert(i == 1); // Text[0]=$abc... is the second lex word
            BWTsa[i] = 0; // eos in BWT, there is no phrase in D corresponding to this
            // symbol so we write dummy values
            if (fputc(0, lastout) == EOF)
                die("bwlast output 2"); // dummy char
            if (args.SAinfo)
                write_myint(0, sa_out); // dummy end of word position, it is never used
            // an 0 does not appear elsewhere in sa_out
        } else {
            if (SA[i] == 1) {
                // BWT[i] = Text[0] = $abcd... = first word in the parsing where $ now
                // plays the role of the EOS in the original text
                if (fputc(last[n - 1], lastout) == EOF)
                    die("bwlast output 3");
            } else {
                if (fputc(last[SA[i] - 2], lastout) == EOF)
                    die("bwlast output 4");
            }
            if (args.SAinfo)
                get_and_write_myint(
                    sa_info, n, SA[i] - 1,
                    sa_out); // ending position of BWT symbol in original text
            BWTsa[i] = Text[SA[i] - 1];
        }
    }
    if (fclose(lastout) != 0)
    ...
    // --- copy BWT to text array (symbol by symbol since sizeof could be
    // different)
    uint32_t *BWT = Text;
    for (long i = 0; i <= n; i++)
        BWT[i] = BWTsa[i];
    ...
    uint32_t *occ = (uint32_t *) malloc(
        (k + 1) * sizeof(*occ)); // extra space for the only occ of 0
    if (occ == NULL)
        die("malloc failed (OCC)");
    FILE *occin = open_aux_file(args.filename, "occ", "rb");
    s = fread(occ + 1, sizeof(*occ), k, occin);
    if (s != k)
        die("not enough occ data!");
    ...
    // ---check
    assert(IList[0] == 1); // EOF is in BWT[1] since P[0] = $xxx is the smallest
    // word and appears once
...
bool FM_Index::pfBWT(const FilePath &fasta_path, const FilePath &output_path,
                     uint_t thread) {
    ...
    // is not in dict
    uint32_t last =
            1; // starting position in ilist of the smallest dictionary word
    ...
    PfBWT::bwt(args, d, dsize, ilist, bwlast, psize, occ,
               dwords); // version not using threads
...
bool FM_Index::read_and_build_sampled_sa(const FilePath &sa_file_path) {
    ...
    if (!file)
        throw std::runtime_error("Cannot open SA file");
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        uint64_t val = (uint64_t(ptr[0])) | (uint64_t(ptr[1]) << 8) |
    ...
    if (position_in_SA != 0) {
        ptr = reinterpret_cast<const uint8_t *>(buffer.data()) +
              (position_in_SA - 1) * 5;
        uint64_t val = (uint64_t(ptr[0])) | (uint64_t(ptr[1]) << 8) |
                       (uint64_t(ptr[2]) << 16) | (uint64_t(ptr[3]) << 24) |
                       (uint64_t(ptr[4]) << 32);

        char last_char = std::visit([val](auto &&manager) {
            using PtrType = std::decay_t<decltype(manager)>;
            if constexpr (std::is_same_v<PtrType, std::unique_ptr<SeqPro::SequenceManager> >) {
                return manager->getSubSequenceGlobal(val, 1)[0];
            } else if constexpr (std::is_same_v<PtrType, std::unique_ptr<SeqPro::MaskedSequenceManager> >) {
                return manager->getOriginalManager().getSubSequenceGlobal(val, 1)[0];
            } else {
    ...
    for (size_t i = 0; i < count;
         i += 32) {
        // Step by 32 to sample every 32nd suffix
        ptr = reinterpret_cast<const uint8_t *>(buffer.data()) +
              (i * 5); // Move the pointer to the right position
...
bool FM_Index::buildIndexUsingBigBWT(const FilePath &output_path,
                                     uint_t thread) {
    FilePath fasta_path = std::visit([](auto &&manager) {
        using PtrType = std::decay_t<decltype(manager)>;
        if constexpr (std::is_same_v<PtrType, std::unique_ptr<SeqPro::SequenceManager> >) {
            return manager->getFastaPath();
        } else if constexpr (std::is_same_v<PtrType, std::unique_ptr<SeqPro::MaskedSequenceManager> >) {
            return manager->getOriginalManager().getFastaPath();
        } else {
            throw std::runtime_error("Unhandled manager type in variant.");
        }
    }, fasta_manager);
...
bool FM_Index::read_and_build_bwt(const FilePath &bwt_file_path) {
    ...
    if (!bwt_file.is_open()) {
        throw std::runtime_error("Error: Cannot open file " +
                                 bwt_file_path.string());
    }

    // 获取文件大小，检查文件大小是否合理
    std::streamsize size = bwt_file.tellg();
    if (size <= 0) {
        throw std::runtime_error("Error: Invalid file size for " +
                                 bwt_file_path.string());
    }
    ...
    if (!bwt.empty()) {
        ...
        throw std::runtime_error("Error: BWT string is empty after reading file " +
                                 bwt_file_path.string());
    }
    ...
    // uint8_t symbol = static_cast<uint8_t>('C');  // 可以换成其他字符如 'C',
    // 'G', 'T' size_t pos = 20; size_t count = wt_bwt.rank(pos, symbol);
