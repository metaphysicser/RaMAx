...
int main(int argc, char** argv) {
	...
	if (common_args.enable_repeat_masking) {
		...
		// 运行 WindowMasker 生成 interval 文件
		std::map<SpeciesName, FilePath> interval_files_map = repeatSeqMasking(common_args.work_dir_path, species_path_map, common_args.thread_num);

		if (interval_files_map.empty()) {
			spdlog::warn("No interval files were generated by repeatSeqMasking. Skipping application of masks.");
		} else {
